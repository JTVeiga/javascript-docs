...
MongoClient.connect(connectionURL, { useNewUrlParser: true, useUnifiedTopology: true }, (err, client) =>{
    ...
    db.collection('users').findOne({ _id: new ObjectID("5ea48e26aff7523451f52011") }, (err, data) => {
        if (err) {
            return console.log('Unable to fetch')   
        }
    ...
    db.collection('users').find( {age: 26}).toArray((err,data) => { // To array return the matching search
        console.log(data);
    })

    db.collection('users').find( {age: 26}).count((err,data) => { // Count search results
        console.log(data);
    })

    db.collection('tasks').findOne({ _id: new ObjectID("5ea48e26aff7523451f52011") }, (err, data) => {
        if (err) {
            return console.log('Unable to fetch')   
        }
    ...
    db.collection('tasks').find( {completed: false}).toArray((err,data) => { // Count search results
        if (err) {
            return console.log('Unable to fetch')   
        }
        console.log(data);
    ...
    db.collection('users').updateOne({
        ...
        $set: {  // "$set" is the operator to set new values
